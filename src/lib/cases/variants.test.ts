...
describe("per-attempt case variants", () => {
  it("is deterministic in the seed and never mutates the authored case", () => {
    ...
    const a = applyCaseVariant(case1, 12345);
    const b = applyCaseVariant(case1, 12345);
    const c = applyCaseVariant(case1, 99999);

    expect(JSON.stringify(a)).toBe(JSON.stringify(b));
    expect(JSON.stringify(case1)).toBe(before);
    // Every variant retains the authored encounter date.
    expect([a.date, c.date].every((d) => /^\d{2}\/\d{2}\/\d{2}$/.test(d))).toBe(true);
  });

  it("keeps every clinical field identical to the authored case", () => {
    const variant = applyCaseVariant(case1, 4242);
  ...
  it("preserves chronology for every case and seed", () => {
    for (const authored of STATIC_CASES) for (const seed of [0, 1, 500, Date.now()]) {
      expect(applyCaseVariant(authored, seed).date).toBe(authored.date);
    }
  });
  ...
  it("rotates the prescriber through the authored pool only", () => {
    ...
    for (let seed = 0; seed < 40; seed += 1) {
      const variant = applyCaseVariant(case1, seed);
  ...
  it("varies approval numbers but never streamlined codes", () => {
    ...
    for (let seed = 0; seed < 10; seed += 1) {
      const variant = applyCaseVariant(case7, seed);
    ...
    const streamlined = applyCaseVariant(case13, 777);
  ...
  it("leaves the prescriber-mismatch teaching case untouched", () => {
    const variant = applyCaseVariant(case9, 31337);
